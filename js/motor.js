...
function inicio() {
  console.log('carregando arquivo... ' + queryString.processo);
...
function mapearProcessos() {
  ...
  for (var i = 0; i < n - 1; i++) {
    processosTemp.push(
      processosClonados.splice(
        Math.floor(Math.random() * processosClonados.length),
        1
      )[0]
    );
  }
...
function imprimirGrupos(indicesJaAdicionados, maximo) {
  console.log(
    'INICIO ----- QUantidade de itens ja adicionados: ' +
      indicesJaAdicionados.length
  );
  var total = 0;
  $('#main').append('<br/>');
  ...
  console.log(
    'FIM ----- QUantidade de itens ja adicionados: ' +
      indicesJaAdicionados.length
  );
}
